...
export function selectBestPerformance(performances, comparison_direction) {
  ...
  const eligibleAndValidated = compliant.filter(p => {
    if (p.validation_status === 'unverified') return false;
    return checkEligibility(p).eligible === true;
  });

  if (eligibleAndValidated.length > 0) {
    return {
      performance: reduceBest(eligibleAndValidated, comparison_direction),
      fallback: false,
    };
  }

  const experimentalPool = compliant.filter(p => p.validation_status === 'experimental');
  if (experimentalPool.length > 0) {
    return {
      performance: reduceBest(experimentalPool, comparison_direction),
      fallback: true,
    };
  }
